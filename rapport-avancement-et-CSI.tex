...
Les \acp{CRDT} sont de nouvelles spécifications des types abstraits de données, tels que l'Ensemble et la Séquence.
...
Ces identifiants représentent donc le surcoût de l'utilisation des \acp{CRDT} par rapport aux structures de données traditionnelles.
Ce surcoût, parfois non-borné, peut décourager l'adoption des \acp{CRDT} par les systèmes distribués.
Le but de cette thèse est donc de proposer des solutions pour pallier à ce problème.
Une approche possible est d'étudier ce sujet comme étant un cas particulier du problème du renommage et de proposer un mécanisme permettant de renommer les identifiants pour réduire leur taille, tout en ne nécessitant aucune coordination entre les différents noeuds.
\\
...
Au cours de la 1ère année, j'ai étudié principalement un \ac{CRDT} donné, \emph{LogootSplit}.
...
Afin de présenter cette contribution à la communauté des \emph{CRDT}, je rédige actuellement un article scientifique.\\
